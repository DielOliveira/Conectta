                ...
                <div class="os-hour-row">
                    ...
                    <div class="os-hour-content">
                        @foreach($ocupados as $item)
                            <button type="button" class="os-appointment {{ $item['atendida'] ? 'attended' : '' }}" wire:click="mountAction('agendamento', { ordem: {{ $item['ordem']->id }} })">@if($item['atendida'])<div class="os-attended-label">Atendida</div>@endif<div class="os-appointment-title">{{ $item['ordem']->numero_formatado }}</div><div>{{ $item['disponibilidade']->tecnico->nome }}</div><div class="os-muted">{{ $item['ordem']->nome_atendimento }} · {{ $item['ordem']->veiculo->placa }}</div></button>
                        @endforeach
...
@else
    ...
    @forelse($agenda->get($dia->toDateString(),collect()) as $item)<div class="os-slot {{ $item['ordem']?'busy':'' }} {{ $item['bloqueio']?'blocked':'' }} {{ $item['atendida']?'attended':'' }}"><div><strong>{{ $item['horario']->format('H:i') }}</strong> · {{ $item['disponibilidade']->tecnico->nome }}</div>@if($item['ordem'])@if($item['atendida'])<div class="os-attended-label">Atendida</div>@endif<button type="button" class="os-week-appointment" wire:click="mountAction('agendamento', { ordem: {{ $item['ordem']->id }} })">{{ $item['ordem']->numero_formatado }}</button><div class="os-muted">{{ $item['ordem']->nome_atendimento }} · {{ $item['ordem']->veiculo->placa }}</div>@elseif($item['bloqueio'])<div class="os-muted">Bloqueado</div>@else<div class="os-muted">Livre</div>@endif</div>@empty<p class="os-muted">Sem disponibilidade cadastrada.</p>@endforelse</section>@endforeach</div>
@endif
</x-filament-panels::page>
