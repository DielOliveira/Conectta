...
<style>.os-tools{display:flex;gap:10px;align-items:end;flex-wrap:wrap}.os-tools label{font-size:12px;font-weight:700}.os-tools input,.os-tools select{display:block;background:white;border:1px solid #d1d5db;border-radius:7px;padding:9px}.os-tools button{background:#d97706;color:white;border:0;border-radius:7px;padding:10px 14px;font-weight:700}.os-days{display:grid;grid-template-columns:repeat(7,minmax(220px,1fr));gap:12px;overflow:auto}.os-day{min-width:220px}.os-day h3{font-weight:800;margin:15px 0 8px}.os-slot{background:white;border:1px solid #e5e7eb;border-left:4px solid #10b981;border-radius:8px;padding:10px;margin-bottom:7px}.os-slot.busy{border-left-color:#d97706}.os-slot a{color:#92400e;font-weight:800;text-decoration:none}.os-muted{font-size:12px;color:#6b7280}.os-daily{margin-top:16px;border:1px solid #e5e7eb;border-radius:10px;overflow:hidden;background:#e5e7eb}.os-hour-row{display:grid;grid-template-columns:92px minmax(0,1fr);gap:1px;margin-bottom:1px}.os-hour-row:last-child{margin-bottom:0}.os-hour{background:#f9fafb;padding:12px;font-weight:800}.os-hour-content{background:white;padding:8px;display:flex;align-items:stretch;gap:8px;flex-wrap:wrap;min-height:76px}.os-appointment{display:block;width:190px;border:1px solid #fecaca;border-left:4px solid #dc2626;border-radius:8px;background:#fef2f2;padding:9px;color:#7f1d1d;text-decoration:none}.os-appointment:hover{background:#fee2e2}.os-appointment-title{font-weight:800}.os-free{min-width:180px;max-width:320px;border:1px solid #bbf7d0;border-left:4px solid #16a34a;border-radius:8px;background:#f0fdf4;padding:9px;color:#166534}.os-free-title{font-size:12px;font-weight:800;text-transform:uppercase}.os-free-names{font-size:12px;line-height:1.35;margin-top:3px}.os-unavailable{align-self:center;color:#9ca3af;font-size:12px}.os-empty{margin-top:16px;background:white;border:1px dashed #d1d5db;border-radius:10px;padding:24px;text-align:center;color:#6b7280}@media(max-width:640px){.os-hour-row{grid-template-columns:70px minmax(0,1fr)}.os-hour{padding:10px 7px}.os-appointment,.os-free{width:100%;max-width:none}}</style>
                ...
                <div class="os-hour-row">
                    ...
                    <div class="os-hour-content">
                        @foreach($ocupados as $item)
                            <a class="os-appointment" href="{{ $this->urlOrdem($item['ordem']->id) }}"><div class="os-appointment-title">{{ $item['ordem']->numero_formatado }}</div><div>{{ $item['disponibilidade']->tecnico->nome }}</div><div class="os-muted">{{ $item['ordem']->cliente->nome }} · {{ $item['ordem']->veiculo->placa }}</div></a>
                        @endforeach
                        @if($tecnicosLivres->isNotEmpty())
                            <div class="os-free"><div class="os-free-title">Livre</div><div class="os-free-names">{{ $tecnicosLivres->implode(', ') }}</div></div>
