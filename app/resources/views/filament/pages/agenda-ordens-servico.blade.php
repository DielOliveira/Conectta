    ...
    @else
        <div class="os-daily">
            @foreach($horarios as $horario)
                ...
                @php($tecnicosLivres=$itensHorario->reject(fn($item)=>$item['ordem'] || $item['bloqueio'])->filter(fn($item)=>$horario->isFuture() && $this->tecnicoPodeReceberOs($item['disponibilidade']->tecnico))->pluck('disponibilidade.tecnico.nome')->unique()->sort()->values())
...
@else
    ...
    @forelse($agenda->get($dia->toDateString(),collect()) as $item)<div class="os-slot {{ $item['ordem']?'busy':'' }} {{ $item['bloqueio']?'blocked':'' }}"><div><strong>{{ $item['horario']->format('H:i') }}</strong> · {{ $item['disponibilidade']->tecnico->nome }}</div>@if($item['ordem'])<button type="button" class="os-week-appointment" wire:click="mountAction('agendamento', { ordem: {{ $item['ordem']->id }} })">{{ $item['ordem']->numero_formatado }}</button><div class="os-muted">{{ $item['ordem']->nome_atendimento }} · {{ $item['ordem']->veiculo->placa }}</div>@elseif($item['bloqueio'])<div class="os-muted">Bloqueado</div>@else<div class="os-muted">Livre</div>@endif</div>@empty<p class="os-muted">Sem disponibilidade cadastrada.</p>@endforelse</section>@endforeach</div>
@endif
</x-filament-panels::page>
